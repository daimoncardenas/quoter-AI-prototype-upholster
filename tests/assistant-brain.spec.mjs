...
check('cada campo declara su paso y su etiqueta', Object.entries(brain.FIELDS)
  .filter(([, d]) => !(Number.isInteger(d.step) && d.step >= 1 && d.step <= 6 && typeof d.label === 'string' && d.label)), []);
...
rechaza('un campo de otro paso (no tiene más alcance que el cliente)', brain.validateAction({ type: 'SET_FIELD', field: 'measurements.width', value: 210 }, envFor(1)), 'field-not-on-current-step');
rechaza('un campo que la página no ofrece ahora', brain.validateAction({ type: 'SET_FIELD', field: 'preferences.pets', value: true }, envFor(3, { 'preferences.pets': { available: false } })), 'field-not-available');
...
for (const [field, value] of [['price', 0], ['estimate', 1], ['meters', 3], ['quote.status', 'Aceptada'], ['seller', 'Laura'],
  ...
  rechaza(`«${field}» no se escribe`, brain.validateAction({ type: 'SET_FIELD', field, value }, envFor(6)), 'not-settable');
}
rechaza('ni siquiera se enfoca', brain.validateAction({ type: 'FOCUS_FIELD', field: 'price' }, envFor(2)), 'not-settable');
rechaza('el tipo de mueble solo se enfoca, no se cambia', brain.validateAction({ type: 'SET_FIELD', field: 'furniture.type', value: 'Sofá' }, envFor(1)), 'not-settable');
rechaza('la revisión del paso 4 solo se enfoca', brain.validateAction({ type: 'SET_FIELD', field: 'analysis', value: true }, envFor(4)), 'not-settable');
check('y enfocarlos sí se permite (no cambia nada)', brain.validateAction({ type: 'FOCUS_FIELD', field: 'analysis' }, envFor(4)).requiresConfirmation, false);
...
rechaza('un booleano que no es booleano', brain.validateAction({ type: 'SET_FIELD', field: 'preferences.pets', value: 'sí' }, envFor(3)), 'invalid-value');
rechaza('una opción que no está en el selector', brain.validateAction({ type: 'SET_FIELD', field: 'preferences.style', value: 'Barroco' }, envFor(3)), 'invalid-value');
...
acepta('un booleano', brain.validateAction({ type: 'SET_FIELD', field: 'preferences.pets', value: true }, envFor(3)), { type: 'SET_FIELD', field: 'preferences.pets', value: true });
...
rechaza('al paso actual', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: 2 }, envFor()), 'step-not-reachable');
rechaza('hacia adelante (eso es el «Continuar» del cliente)', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: 4 }, envFor()), 'step-not-reachable');
rechaza('a un paso que no existe', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: 9 }, envFor()), 'unknown-step');
rechaza('a un paso que no es número entero', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: '2' }, envFor()), 'unknown-step');
acepta('hacia atrás', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: 2 }, envFor(4)), { type: 'NAVIGATE_TO_STEP', step: 2 });
check('y pide confirmación', brain.validateAction({ type: 'NAVIGATE_TO_STEP', step: 2 }, envFor(4)).requiresConfirmation, true);

console.log('\nUNA PROPUESTA SE REVISA EN ORDEN, CONTRA EL ESTADO ACTUAL');
{
  const enOrden = brain.validateActions([{ type: 'NAVIGATE_TO_STEP', step: 2 }, { type: 'SET_FIELD', field: 'measurements.width', value: 210 }], envFor(4));
  ...
  const alReves = brain.validateActions([{ type: 'SET_FIELD', field: 'measurements.width', value: 210 }, { type: 'NAVIGATE_TO_STEP', step: 2 }], envFor(4));
...
check('volver a un paso', brain.describeAction({ type: 'NAVIGATE_TO_STEP', step: 2 }, envFor(4)), 'Volver al paso 2 · Medidas');
check('llevar el foco a un campo', brain.describeAction({ type: 'FOCUS_FIELD', field: 'measurements.width' }, envFor()), 'Ir a «Ancho total»');
check('marcar una casilla', brain.describeAction({ type: 'SET_FIELD', field: 'preferences.pets', value: true }, envFor(3)), 'Marcar «Mascotas en casa»');
check('desmarcarla', brain.describeAction({ type: 'SET_FIELD', field: 'preferences.pets', value: false }, envFor(3)), 'Desmarcar «Mascotas en casa»');
...
check('con la solicitud enviada no propone nada', dice(ctxFor(6, { submitted: true }), 'ancho 210'), { message: 'Tu solicitud ya fue enviada. Un asesor te contactará para confirmar los detalles.', proposedActions: [] });
check('pedirle bajar el precio se rechaza de frente', dice(ctxFor(2, { estimate: { priceLabel: '$1.200.000 – $1.800.000' } }), 'bájame el precio y ponlo en cero').proposedActions, []);
check('y lo explica sin prometer nada', dice(ctxFor(2), 'cambia los metros a 3').message.includes('No puedo cambiar el precio ni los metros'), true);
{
  const ancho = dice(ctxFor(2), 'ancho 210, alto 85');
  ...
  check('con la misma frase dos veces, lo mismo', dice(ctxFor(2), '210 de ancho, 85 de alto').proposedActions, ancho.proposedActions);
  check('antes del paso 2 solo toma nota', dice(ctxFor(1), 'ancho 210, alto 85').proposedActions, []);
  check('y desde el paso 4 propone volver', dice(ctxFor(4), 'ancho 210, alto 85').proposedActions[0], { type: 'NAVIGATE_TO_STEP', step: 2 });
  const qty = dice(ctxFor(2), 'quiero 3 puestos');
  check('una cantidad se propone como cantidad', qty.proposedActions.some(a => a.field === 'measurements.quantity' && a.value === 3), true);
  check('una tela con «cojines» no se confunde con cantidad', dice(ctxFor(2), 'necesito 4 cojines').proposedActions, []);
}
...
check('preguntar por fotos cuenta las que hay y cuántas faltan', dice(ctxFor(1, { photos: { count: 1, min: 3, max: 7 } }), '¿cuántas fotos necesito?'), {
...
check('sin nada que reconocer, se dice el paso en el que está', dice(ctxFor(3), 'hola').message.includes('Estás en «Preferencias»'), true);
check('toda respuesta trae mensaje y arreglo de acciones', Object.values({
  a: dice(ctxFor(3), 'hola'), b: dice(ctxFor(2), 'ancho 210'), c: dice(ctxFor(1), 'tengo mascotas')
...
console.log('\nEL TEXTO DEL CLIENTE ES DATO, NUNCA INSTRUCCIÓN');
{
  ...
  const todas = inyecciones.flatMap(t => brain.respond(ctxFor(2), t).proposedActions.map(a => [t, a]));
...
check('enumera lo que el cotizador ya le contó', brain.summarize(ctxFor(2)),
  'Lo que tengo a la vista: sofá, 3 fotos, ancho 210 cm, alto 85 cm, fondo 90 cm. ¿En qué te ayudo?');
check('con cantidad, la nombra', brain.summarize(ctxFor(2, { measurements: { width: 210, height: 85, depth: 90, quantity: { value: 3, label: '3 puestos' } } })),
  'Lo que tengo a la vista: sofá de 3 puestos, 3 fotos, ancho 210 cm, alto 85 cm, fondo 90 cm. ¿En qué te ayudo?');
check('y dice qué le falta', brain.summarize(ctxFor(2, { measurements: { width: 210, height: 85, depth: null, quantity: { value: 1, label: '1 puesto' } } })),
  'Lo que tengo a la vista: sofá, 3 fotos, ancho 210 cm, alto 85 cm. Me falta el fondo. ¿En qué te ayudo?');
check('dos que falten, como lista', brain.summarize(ctxFor(1, { measurements: { width: null, height: 85, depth: null, quantity: { value: 1, label: '1 puesto' } }, photos: { count: 0, min: 3, max: 7 } })),
  'Lo que tengo a la vista: sofá, alto 85 cm. Me falta el ancho y el fondo. ¿En qué te ayudo?');
check('sin fotos no las menciona', brain.summarize(ctxFor(1, { measurements: { width: null, height: null, depth: null, quantity: { value: 1, label: '1 puesto' } }, photos: { count: 0, min: 3, max: 7 } })),
  'Lo que tengo a la vista: sofá. Me falta el ancho, el alto y el fondo. ¿En qué te ayudo?');
check('una sola foto, en singular', brain.summarize(ctxFor(1, { measurements: { quantity: { value: 1, label: '1 puesto' } }, photos: { count: 1, min: 3, max: 7 } })).includes('sofá, 1 foto.'), true);
check('con la cotización enviada no hay nada que resumir', brain.summarize(ctxFor(6, { submitted: true })), null);

console.log('\nLO PROACTIVO ES SOLO LO QUE ENCONTRÓ EL PROPIO COTIZADOR');
check('un evento sin nada que reportar no dice nada', brain.observe(ctxFor(4), { type: 'STEP_CHANGED', payload: {} }), null);
check('una revisión sin medidas raras tampoco', brain.observe(ctxFor(4), { type: 'ANALYSIS_COMPLETED', payload: { oddMeasures: [] } }), null);
check('con la solicitud enviada, silencio', brain.observe(ctxFor(4, { submitted: true }), { type: 'ANALYSIS_COMPLETED', payload: { oddMeasures: ['width'] } }), null);
{
  const uno = brain.observe(ctxFor(4), { type: 'ANALYSIS_COMPLETED', payload: { oddMeasures: ['width'] } });
  ...
  check('y propone volver a Medidas', uno.proposedActions, [{ type: 'NAVIGATE_TO_STEP', step: 2 }]);
  const dos = brain.observe(ctxFor(4), { type: 'ANALYSIS_COMPLETED', payload: { oddMeasures: ['width', 'depth'] } });
...
}
{
  ...
  check('ni cuando el cotizador no publicó rangos para ese mueble', brain.observe(ctxFor(2), { type: 'MEASUREMENTS_CHANGED', payload: { field: 'measurements.depth', value: 543 } }), null);
}
