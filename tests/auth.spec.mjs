...
const signIn = async (p, email, pw='upholster-prototype-quoter') => {
...
await signIn(p, 'maria@upholster-prototype-quoter.com', 'incorrecta');
...
await signIn(p, 'noexiste@upholster-prototype-quoter.com', 'upholster-prototype-quoter');
check('mismo mensaje para un correo inexistente', await p.textContent('#loginError'), 'Correo o contraseña incorrectos');

console.log('\nADMIN VE TODO');
await signIn(p, 'maria@upholster-prototype-quoter.com');
...
await signIn(p2, 'laura@upholster-prototype-quoter.com');
...
check('el seed no deja vendedores pausados con la cuenta abierta',
  await p.evaluate(()=>Store.all('sellers').filter(v=>!v.active)
    ...
    .map(v=>v.name)), []);
await signOut(p);
await signIn(p, 'daniel@upholster-prototype-quoter.com');
...
await p.fill('#sellerForm [name=email]','sofia@upholster-prototype-quoter.com');
...
check('se le crea usuario de vendedor',
  await p.evaluate(()=>{const u=Store.all('users').find(x=>x.email==='sofia@upholster-prototype-quoter.com');return u?u.role:'sin usuario'}),
  'seller');
await signOut(p);
await signIn(p, 'sofia@upholster-prototype-quoter.com');
...
check('y su cuenta se cierra con él',
  await p.evaluate(()=>Store.all('users').find(u=>u.email==='sofia@upholster-prototype-quoter.com').active), false);
...
check('aclara que no es el sitio oficial y de quién es el diseño', await (async()=>{
  ...
  return ['Prototipo demostrativo','no es la página oficial de upholster-prototype-quoter',
...
check('y no expone credenciales: se envían aparte con la propuesta', await (async()=>{
  const t=await p.textContent('#demoBanner');
  return !/@upholster-prototype-quoter\.com/.test(t) && !t.includes(await p.evaluate(()=>Auth.DEMO_PASSWORD));})(), true);
...
await signIn(pe, 'maria@upholster-prototype-quoter.com');
...
check('y llega con los datos que ya tiene',
  ...
  ['Laura Méndez','laura@upholster-prototype-quoter.com',['sp-12-de-octubre'],'true']);
...
await pe.fill('#sellerForm [name=email]', 'andres@upholster-prototype-quoter.com');
...
check('y no guardó nada', await pe.evaluate(()=>Store.get('sellers',1).email), 'laura@upholster-prototype-quoter.com');

await pe.fill('#sellerForm [name=name]', 'Laura Méndez Ruiz');
await pe.fill('#sellerForm [name=email]', 'laura.ruiz@upholster-prototype-quoter.com');
...
check('guarda nombre, correo y los puntos que cubre',
  ...
  ['Laura Méndez Ruiz','laura.ruiz@upholster-prototype-quoter.com',['sp-12-de-octubre',sopoId].sort()]);
...
check('su cuenta se mueve con él: mismo nombre, mismo correo',
  ...
  ['Laura Méndez Ruiz','laura.ruiz@upholster-prototype-quoter.com']);
...
await signOut(pe);
await signIn(pe, 'laura.ruiz@upholster-prototype-quoter.com');
...
await signOut(pe);
await signIn(pe, 'laura@upholster-prototype-quoter.com');
