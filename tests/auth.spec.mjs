...
check('solo ve resumen y cotizaciones',
  ...
  ['dashboard','quotes']);
check('catálogo, muebles, vendedores, puntos, configuraciones, estilos, asistente, upgrade y usage quedan fuera',
  await p2.$$eval('.nav button', bs => bs.filter(x=>x.hidden).map(x=>x.dataset.page)),
  ['fabrics','furniture','sellers','points','settings','styles','assistant','upgrade','usage']);
